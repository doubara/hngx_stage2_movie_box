    ...
    return (
        <div className="w-full h-auto text-black">
            <div className="flex items-center my-6 px-8 pb-4 border-b">
                <p className="text-xl">Search Results
                </p>

                <button onClick={()=>{
                    navigate('/')
                }} className="ml-auto bg-red-600 text-white px-4 py-3 rounded-lg text-md"> Go Back home</button> 
            </div>
            ...
            {!isLoading && !error.status && !movieData.length && !movieData.total_results && <ErrorPage error={error}></ErrorPage>}
        </div>
